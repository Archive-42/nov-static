...
describe("The merchants table", () => {
  context("when created", () => {
    ...
    it("has id, merchant_name, country_id, created_at, admin_id, and merchant_type_id columns with proper specs", async () => {
      if (!client) {
        expect.fail(
          "Cannot connect to localhost database with merchant_app/bhKx5P6V"
        );
      }
      if (createError) {
        expect.fail(
          `Error while running ${createError.sqlPath}: ${createError.e.message}`
        );
      }

      const {
        rows: [{ id: merchantTypeId }],
      } = await client.query(`
